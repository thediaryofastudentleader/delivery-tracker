...
export default function ProductSection({
  ...
  liked, toggleLike, addToCart, hotSale, cart, cpcCategory
}) {
  const showBulkBadge = cpcCategory !== 4;
          ...
          {CATEGORIES.map(cat => {
            ...
            return (
              <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} style={{
                ...
                background: isActive ? 'linear-gradient(135deg, #10b981, #0d9488)' : (dark ? 'rgba(51, 65, 85, 0.6)' : 'rgba(255,255,255,0.8)'),
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {filteredProducts.map(product => {
            ...
            const hasBulkInCart = cartItem && cartItem.qty >= 10 && cpcCategory !== 4;
            ...
            return (
              <div key={product.id} style={{
                ...
                <div style={{ position: 'relative', aspectRatio: '1', background: dark ? '#1e293b' : '#f1f5f9', overflow: 'hidden' }}>
                  <Image src={product.image} alt={product.title} fill style={{ objectFit: 'cover' }} sizes="(max-width: 768px) 50vw, 200px" />
                  {product.special && (
                    <div style={{ position: 'absolute', top: 10, left: 10, padding: '4px 12px', borderRadius: 12, background: 'linear-gradient(90deg, #ec4899, #f43f5e)', color: '#fff', fontSize: 10, fontWeight: 700, zIndex: 2, boxShadow: '0 4px 12px rgba(236, 72, 153, 0.3)' }}>SPECIAL</div>
                  )}
                  {product.new && (
                    <div style={{ position: 'absolute', top: 10, left: 10, padding: '4px 12px', borderRadius: 12, background: 'linear-gradient(90deg, #10b981, #14b8a6)', color: '#fff', fontSize: 10, fontWeight: 700, zIndex: 2, boxShadow: '0 4px 12px rgba(16, 185, 129, 0.3)' }}>NEW</div>
                  )}
                  {isOnSale && (
                    <div style={{ position: 'absolute', top: 10, right: 10, padding: '4px 12px', borderRadius: 12, background: 'linear-gradient(90deg, #f97316, #f59e0b)', color: '#fff', fontSize: 10, fontWeight: 700, zIndex: 2, boxShadow: '0 4px 12px rgba(249, 115, 22, 0.3)' }}>-{hotSale.discount}%</div>
                  )}

                  {/* Bulk Promo Badge — Hidden for Cat 4 */}
                  {showBulkBadge && (
                    <div style={{
                  ...
                  <button onClick={() => toggleLike(product.id)} style={{
                    position: 'absolute', top: 10, right: isOnSale ? 60 : 10,
                ...
                <div style={{ padding: '14px 16px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    ...
                    {isOnSale && <span style={{ fontSize: 13, color: dark ? '#64748b' : '#94a3b8', textDecoration: 'line-through' }}>R{product.price.toFixed(2)}</span>}
                  </div>

                  {/* Bulk Promo Message — Hidden for Cat 4 */}
                  {hasBulkInCart && showBulkBadge && (
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 11, color: currentStock > 5 ? '#10b981' : currentStock > 0 ? '#f59e0b' : '#ef4444', fontWeight: 600 }}>
                      {currentStock > 0 ? `${currentStock} left` : 'Out of stock'}
                    </span>
                    <button onClick={() => addToCart(product.id)} disabled={!inStock} style={{
